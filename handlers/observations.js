...
async function PostObservationsHandler(req, res) {
    try {
        ...
        const volumeUsagePercentage = parseFloat(
            req.body.environment.storage.find((item) => item.mounted_on === "/")
                .use_percentage
        );
        const memoryUsagePercentage =
            parseFloat(
                req.body.environment.memory.used /
                    req.body.environment.memory.total
            ) * 100;
        ...
        if (
            req.body.logs &&
            (req.body.logs.includes("ERROR") ||
                req.body.logs.includes("WARNING"))
        ) {
            dangers.push("logs");
        }
